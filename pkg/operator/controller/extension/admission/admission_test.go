...
var _ = Describe("Deployment", func() {
	...
	Describe("#Reconcile", func() {
		...
		It("should fail when virtual OCI artifact is not found", func() {
			Expect(admission.Reconcile(ctx, log, virtualClientSet, genericKubeconfigSecretName, extension)).To(MatchError(`failed pulling Helm chart from OCI repository: artifact local-extension-virtual:v1.2.3 not found`))
		})
		...
		It("should fail when runtime OCI artifact is not found", func() {
			...
			Expect(admission.Reconcile(ctx, log, virtualClientSet, genericKubeconfigSecretName, extension)).To(MatchError(`failed pulling Helm chart from OCI repository: artifact local-extension-runtime:v1.2.3 not found`))
