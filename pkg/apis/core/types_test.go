...
var _ = Describe("types", func() {
	...
	Describe("Capabilities", func() {
		...
		BeforeEach(func() {
			capabilities = Capabilities{"fooCap": CapabilityValues{Values: []string{"a", "b", "c"}}}
		})

		Describe("#HasEntries", func() {
			It("should return true if it has capabilities defined", func() {
				Expect(capabilities.HasEntries()).To(BeTrue())
			})

			It("should return false if it hasn't capabilities defined", func() {
				Expect((Capabilities{}).HasEntries()).To(BeFalse())
			})
		})
	})
})
